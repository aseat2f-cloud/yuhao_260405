          ...
          {variant === 'elementary' ? (
            <div key={gridKey} className="flex flex-col items-center gap-6 sm:gap-10">
              {ELEMENTARY_IMAGE_DATA[activeTab as keyof typeof ELEMENTARY_IMAGE_DATA]?.map((img, idx) => (
                  <div 
                    key={idx} 
                    className={`w-full bg-white rounded-2xl overflow-hidden shadow-xl border border-slate-100 transform transition-all duration-700 aspect-[1.4/1] sm:aspect-auto sm:min-h-[400px] ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}
