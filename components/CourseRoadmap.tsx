...
const CourseRoadmap: React.FC = () => {
  ...
  const COURSE_DATA: CourseCategory[] = [
    {
      ...
      classes: [
        ...
        { name: '外師發音班', desc: '由專業外師親自引導純正口音，掌握自然發音 (Phonics) 技巧，讓孩子看到單字能讀、聽到單字能拼，奠定語感。', age: '小一 ~ 小五', time: '週二或週四 晚上', objectives: '建立正確發音嘴型與聽音辨位能力，看到單字能讀，聽到單字能拼。', target: '初學美語 or 希望矯正發音的孩子。', features: ['專業外師親自授課', '趣味發音遊戲與歌謠', '矯正台式發音習慣', '建立拼讀自信心'], roadmap: ['Level A: 字母與短母音', 'Level B: 長母音與混合音', 'Level C: 特殊發音規則'] },
    ...
    },
    {
      ...
      classes: [
        ...
        { 
          name: '種子超前數學班', 
          desc: '專為潛力學子設計，透過啟發式引導激發數學潛能，培養深厚的邏輯思考種子，為未來的資優學習之路奠定堅實基礎。', 
          age: '小五', 
          time: '週六',
        ...
        { 
          name: '超前數學班', 
          desc: '鎖定私中及公校資優班目標，挑戰高階思維題型與奧數邏輯，訓練極致的解題速度與精準度，全面提升學生的競爭實力。', 
          age: '小六', 
          time: '週六',
  ...
  return (
    <section ref={sectionRef} className="py-24 bg-green-600 relative overflow-hidden">
      {/* Texture Background */}
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="flex flex-col md:flex-row gap-3 md:gap-8">
          ...
          <div className="md:w-3/4">
            ...
            {/* Class Cards or Images */}
            <div className={`animate-in fade-in slide-in-from-right-4 duration-300 ${COURSE_DATA[activeTab].displayType === 'images' ? 'space-y-6' : 'grid grid-cols-1 sm:grid-cols-2 gap-4'}`}>
              {COURSE_DATA[activeTab].displayType === 'images' ? (
                <div className="flex flex-col gap-6">
                  {COURSE_DATA[activeTab].images?.map((url: string, i: number) => (
                    <img 
                      key={i} 
                      src={url} 
                      alt={`艾森樂小玩家 ${i + 1}`} 
                ...
                <div className="flex items-center gap-2">
                   ...
                   <div className="text-slate-900 font-medium">
                     {renderTimeWithCircles(
                       selectedClass.name === '進度數學班' 
                         ? (selectedGrade === '三年級' ? '週三 下午' : (selectedGrade === '四年級' ? '週二五 晚上' : (selectedGrade === '五年級' || selectedGrade === '六年級' ? '週二五 和 六' : selectedClass.time)))
                         : selectedClass.time
                     )}
                   </div>
                </div>
